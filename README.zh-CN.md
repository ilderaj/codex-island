...
- 在展开的 **Usage** 中打开 **账号**，可以比较已保存快照并暂存一个非当前账号。
  浏览、暂存或取消都不会修改当前认证。
- 使用账号 rail 右侧的刷新图标，可以在不修改当前认证的情况下刷新所有已保存账号的用量。
- **切换并重启 ChatGPT** 必须经过显式确认。它会在本地认证切换前、以及尝试重启前
  分别验证精确的 `/Applications/ChatGPT.app` 目标。即使已尝试启动，界面仍会保持
  “认证重载未验证”，直到新的用量读取证明认证已生效。
- 如果本地切换后重启失败，应用可以恢复上一个本地快照；在没有新证据前，不会声称
  ChatGPT 已接受新认证或恢复后的认证。
...
- 点击展开面板左下角的齿轮打开设置。
- 在展开的 **Usage** 中使用 **账号** 查看本地账号用量，并使用 rail 右侧的刷新图标
  刷新所有已保存账号。只有在准备让 ChatGPT 退出并重新打开时，才确认
  **切换并重启 ChatGPT**；关闭确认提示不会产生修改。
